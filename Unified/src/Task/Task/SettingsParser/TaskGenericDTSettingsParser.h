...
struct TaskSettings
{
  ...
  TaskSettings(): destinationTime(-1.0) {}

  TaskSettings(LambdaSettings<Space>* genericDTParser): genericDTParser(genericDTParser), destinationTime(-1.0) {}
  ...
  struct BoxStateInfo
  {
    ...
    Vector boxPoint1;
    Vector boxPoint2;
    GenericDataType vectorVelocity;
    IndexType submeshToApply;
  };
  std::vector<BoxStateInfo> boxStateInfos;

  struct BoxRotationStateInfo
  {
    BoxRotationStateInfo():
      boxPoint1(Vector::zeroVector()),
      boxPoint2(Vector::zeroVector()),
      pos(Vector::zeroVector()),
      angularVelocity(0.0),
      rotationAxis(Vector3::zeroVector()),
      linearSpeed(Vector::zeroVector()),
      submeshToApply(0) {}
    Vector boxPoint1;
    Vector boxPoint2;
    Vector pos;
  ...
  std::vector<BoxRotationStateInfo> boxRotationStateInfos;
...
void TaskSettings<Space>::Parse(TiXmlElement *taskElement)
{
  ...
  if(iniStateElement)
  {
    ...
    while(boxStateElement)
    {
      ...
      genericDTParser->ParseToGenericDT(boxStateElement, "velocity",   &(boxInfo.vectorVelocity ));
      ParseUnsigned(boxStateElement, "submeshToApply", &(boxInfo.submeshToApply));
    ...
    TiXmlElement *boxRotationStateElement = iniStateElement->FirstChildElement("BoxRotationState");
    while(boxRotationStateElement)
    {
      IniState state;
      state.type = IniState::BoxRotation;
      state.infoIndex = boxRotationStateInfos.size();

      BoxRotationStateInfo boxRotationInfo;
      ParseVector(boxRotationStateElement, "boxPoint1",  &(boxRotationInfo.boxPoint1));
      ParseVector(boxRotationStateElement, "boxPoint2",  &(boxRotationInfo.boxPoint2));
      ParseVector(boxRotationStateElement, "pos", &(boxRotationInfo.pos));
      ParseScalar(boxRotationStateElement, "angularVelocity", &(boxRotationInfo.angularVelocity));
      ParseVector(boxRotationStateElement, "rotationAxis", &(boxRotationInfo.rotationAxis));
      ParseVector(boxRotationStateElement, "linearVelocity", &(boxRotationInfo.linearSpeed));
      ParseUnsigned(boxRotationStateElement, "submeshToApply", &(boxRotationInfo.submeshToApply));
      boxRotationStateInfos.push_back(boxRotationInfo);

      iniStates.push_back(state);

      boxRotationStateElement = boxRotationStateElement->NextSiblingElement("BoxRotationState");
    }
